...
class NumericalGeodesics():
    ...
    def __init__(self, n_max, step_count):
        ...
        # Mode2 = Shooting mode = Zero initial speed
        basis = schauder_basis( self.n_max, self.time_grid.numpy() )
        basis = torch.t( torch.tensor( basis ) )
        basis = basis[:,1:] # Throw away the first basis vector, the constant function
        ...
        self.schauder_bases["zero_initial_speed"] = { "basis": basis, "N_max": N_max}
    ...
    def computeGeodesicShooting(self, generator, b1, b2, f1, v1, epochs, optimizer_info, display_info):
        ...
        N_max_2 = self.schauder_bases["zero_initial_speed"]["N_max"]
        basis2  = self.schauder_bases["zero_initial_speed"]["basis"]
        # Dimensions
        base_dim     = b1.shape[0]
        ...
        # parameters2: Coefficients of fiber curve in Schauder basis, with zero initial speed
        ...
        with tqdm( range(epochs) ) as t:
            for i in t:
                ...
                t.set_description( "%s. Energy %f, Base grad %f, Fiber grad %f"%(display_info, energy, grad1_norm, grad2_norm) )
                optimizer.step()
            # End for
        # End with
        #
        ...
        return naive_curve.detach().numpy(), geodesic_curve.detach().numpy()