...
class ConditionHandler(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.embed = torch.nn.Embedding(nb_class, base_dim)
    ...
    def forward(self, cond):
        return self.embed( cond.long() ).view((-1, self.base_dim))

    def predict_fiber_condition(self, fiber_out):
        fiber_out = grad_reverse(fiber_out)
        prediction = self.fiber_condition_adv(fiber_out)
        return prediction

    def predict_condition(self, x_out):
